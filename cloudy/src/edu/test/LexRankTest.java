...
import edu.cloudy.nlp.ranking.LexRankResults;
import edu.cloudy.nlp.ranking.LexRanker;
import edu.cloudy.nlp.ranking.Similar;
...
public class LexRankTest 
{
    ...
    public void testLexRank() 
    {
        ...
        List<String> names = Arrays.asList("d1s1", "d2s1", "d2s2", "d2s3", "d3s1", "d3s2", "d3s3", "d4s1", "d5s1", "d5s2", "d5s3", "d5s4");
        double max = results.scores.get(results.rankedResults.get(0));
        
        for (int i = 0; i < similarity.length; ++i)
        {
            System.out.println(names.get(i) + ": " + (results.scores.get(items.get(i)) / max));
        }
    }
